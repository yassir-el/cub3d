...
void	ft_init_textures_3(char *str, char *str_2, char *str_3, t_cub3d *cub)
{
	int	i;
	...
	{
		str = ft_itoa(i + 1);
		str_2 = ft_strjoin("textures/door/", str);
		free(str);
		str_3 = ft_strjoin(str_2, ".xpm");
		free(str_2);
		if (str_3 == NULL)
			ft_print_err(NULL, 1);
		cub->data.door[i].img = mlx_xpm_file_to_image(\
			cub->data.mlx_ptr, str_3, &cub->data.door[i].width, \
			&cub->data.door[i].height);
		if (cub->data.door[i].img == NULL)
			ft_error_("Error\nError In XPM to IMG file door\n");
		free(str_3);
	}
	i = -1;
	while (++i < F_DOOR)
		cub->data.door[i].addr = mlx_get_data_addr(cub->data.door[i].img, \
			&cub->data.door[i].bits_per_pixel, &cub->data.door[i].size_line, \
				&cub->data.door[i].endian);
}
...
void	ft_init_textures_2(t_cub3d *cub)
{
	...
	ft_init_textures_3(str, str_2, str_3, cub);
}
