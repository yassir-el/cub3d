...
void	cast_rays(float rayAngle, t_cub3d *cub, int id)
{
	...
	if (help.start < 0)
		help.start = 0;
	if (help.end >= WIN_HEI - 1)
		help.end = WIN_HEI - 1;
	render_wall(id, help.start, help.end, cub);
}

void	cast_rays__2(float rayAngle, t_cub3d *cub, int id)
{
	t_help	help;

	horizontal__2(rayAngle, cub, &help.next_x_1, &help.next_y_1);
	vertical__2(rayAngle, cub, &help.next_x_2, &help.next_y_2);
	if (map_in(help.next_y_1, help.next_x_1, cub) != 'M' && \
					map_in(help.next_y_2, help.next_x_2, cub) != 'M')
		return ;
	ft_help(&help, cub, rayAngle);
	if (help.start < 0)
		help.start = 0;
