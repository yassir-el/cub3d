...
void	check_line_2(t_list *head, char *prev, int *f, int *p_count)
{
	...
	while (head->data[++i])
	{
		if (head->data[i] == ' ' || head->data[i] == '\n')
			...
			*f = 1;
		else if (head->data[i] == '0' || head->data[i] == 'M')
...
void	ft_check_line_3(t_list *head, char *prev, int *f, int i)
{
	...
	if ((!*f || !head->next))
		ft_print_err("wrong place of '0' or P or M\n", 0);
...
void	check_mp(t_list *head, char *prev, int len_next, int j)
{
	...
	if (j >= len_prev || j >= len_next)
		ft_print_err("wrong char surrounding 0 or M or P up and down\n", 0);
	if (prev[j] == ' ' || head->next->data[j] == ' '
		|| head->data[j + 1] == ' ')
		ft_print_err("wrong char surrounding 0 or M or P\n", 0);
	if (head->data[j] == 'M')
	{
		ok = (((prev[j] == '1' || prev[j] == 'M') \
				&& (head->next->data[j] == '1' \
				|| head->next->data[j] == 'M')) \
				|| ((head->data[j - 1] == '1' || head->data[j - 1] == 'M') \
				&& (head->data[j + 1] == '1' || head->data[j + 1] == 'M')));
		if (!ok)
			ft_print_err("wrong char surrounding M\n", 0);
	}	
}
